using HypothesisTests, Test, Distributions
...
@testset "Friedman" begin
    ...
    @testset "iman_davenport" begin
        t2 = FriedmanTest(b1, b2, b3)
        F, p = iman_davenport(t2)
        # F = ((n-1)*Q) / (n*(k-1) - Q), p from F(k-1, (k-1)*(n-1))
        n, k, Q = t2.n, t2.k, t2.Q
        @test F ≈ ((n - 1) * Q) / (n * (k - 1) - Q)
        @test p ≈ 1 - cdf(FDist(k - 1, (k - 1) * (n - 1)), F)
        @test 0.0 < p < 1.0
        @test F > 0.0
    end
end
